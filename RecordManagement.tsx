...
export const RecordManagement = ({ fuelRecords, onUpdateFuelRecords, aircraftData, tankerData, airportList, showMessage }: RecordManagementProps) => {
    ...
    const handleExport = (format: 'xlsx' | 'pdf') => {
        ...
        const data = displayedRecords.map(record => [
            record.kayitNumarasi, record.date, record.personnelName, record.jobTitle || '-',
            record.locationType, record.location, record.receiptNumber, record.tailNumber || '-',
            record.cardNumber || '-', record.fuelAmount
        ]);
